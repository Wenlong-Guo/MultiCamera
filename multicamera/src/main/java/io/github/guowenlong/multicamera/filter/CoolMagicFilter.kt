 ...
 */
class CoolMagicFilter(context: Context) : BaseFilter() {
    private val mToneCurveTexture = intArrayOf(-1)
    private var mToneCurveTextureUniformLocation = 0

    init {
        clear()
        createProgram(context, "cool_vs.glsl", "cool_fs.glsl")
...
//        glMatrixCoord = glGetUniformLocation("vMatrixCoord")
        glInputTextureCoordinate = glGetAttribLocation("inputTextureCoordinate")
        initBuffer()
        customInit()
    }
    ...
    override fun onDraw(mtx: FloatArray, textureId: Int, cameraId: Int) {
        ...
        if (cameraId == 0) {
            ...
            GLES20.glVertexAttribPointer(glInputTextureCoordinate, 2, GLES20.GL_FLOAT, false, 0, backTextureBuffer)
            GLES20.glEnableVertexAttribArray(glInputTextureCoordinate)
        } else {
            frontTextureBuffer?.position(0)
            GLES20.glVertexAttribPointer(glInputTextureCoordinate, 2, GLES20.GL_FLOAT, false, 0, frontTextureBuffer)
            GLES20.glEnableVertexAttribArray(glInputTextureCoordinate)
        }
        ...
        GLES20.glDisableVertexAttribArray(glInputTextureCoordinate)
